...
export default function comicgen(selector, options) {
  ...
  Array.from(selector).forEach((node) => {
    ...
    var svg = [
      `<svg viewBox="${-attrs.x} ${-attrs.y} ${format.width} ${
        format.height
      }" width="${attrs.width || format.width}" height="${
        attrs.height || format.height
      ...
      if (attr in format.files) {
        ...
        if (row.continuous) {
          files[attrs["name"]][attr].forEach((filename) => {
            ...
            let img = row.file.replace(/\$([a-z]*)/g, (match, group) =>
              group === row.continuous ? filename : attrs[group]
            );
            continuousUrls.push({
              fetch: fetch(`${comicgen.base}svg/${img}.svg`).then((res) =>
                res.text()
              ),
          ...
          debugger;
          if (row.file) {
            var img = row.file.replace(
              /\$([a-z]*)/g,
              (match, group) => attrs[group]
            );
            svg.push(
              `<image width="${row.width}" height="${row.height}" transform="translate(${row.x},${row.y})" xlink:href="${comicgen.base}${attrs.ext}/${img}.${attrs.ext}"/>`
            );
          }
          if (row.editText) {
            console.log(attrs);
    ...
    continuousUrls.length &&
      Promise.all(continuousUrls.map((d) => d.fetch)).then((svg_responses) => {
        ...
        for (let i = 0; i < continuousUrls.length; i = i + 2) {
          character_svg_container.innerHTML += `<g>${
            svg_responses[i]
          ...
          create_parametric_svg(
            character_svg_container.querySelector(
              `svg g:nth-of-type(${i / 2 + 1})`
            ),
            continuousUrls[i].sliderVal
          );
        }
      });

    if (options.name == "speechbubbles") {
      var input_text = document.querySelector(".edit-text");
      console.log("attr", input_text);
      input_text.addEventListener("change", () =>
        comicgen(nodeselecor, options)
      );
      var speech_text = document.querySelector(".edit-text").value;
      console.log(speech_text, "text");
      svg.push(`<text x="10" y="10" dy=".35em">${speech_text}</text>`);
    }
    // Add the SVG footer
    svg.push("</g></svg>");
...
function create_parametric_svg(node, sliderVal) {
  ...
  Array.from(character_svg_nodes).forEach((character_svg_node) => {
    ...
    let startnode = node.querySelector(
      `template:nth-of-type(1) #${character_svg_node.id}`
    );
    let endnode = node.querySelector(
      `template:nth-of-type(2) #${character_svg_node.id}`
    );
    Array.from(character_svg_node.attributes)
      .map((d) => d.nodeName)
      .forEach((attr) =>
        character_svg_node.setAttribute(
          attr,
          attr === "d"
            ? // For smoother paths and worse performance, reduce "maxSegmentLength" value (defaults to 10).
              flubber.interpolate(
                startnode.getAttribute(attr),
                endnode.getAttribute(attr),
                { maxSegmentLength: 5 }
              )(sliderVal)
            : d3.interpolate(
                startnode.getAttribute(attr),
...
else window.setTimeout(comicgen);
